...
cpi_raw <- getBLSFiles("cpi", "rortybomb@gmail.com")
...
pce_density_values <- NIPA_monthly_data %>%
  ...
  mutate(Pchange1a = (value / lag(value,1))^12 - 1,
         ...
         Pchange6a = (value / lag(value,6))^2 - 1,
...
# Update last_updated timestamp in global.R
global_lines <- readLines("global.R")
global_lines <- sub(
  'last_updated <- ".*"',
  paste0('last_updated <- "', format(Sys.Date(), "%B %e, %Y"), '"'),
  global_lines
)
writeLines(global_lines, "global.R")